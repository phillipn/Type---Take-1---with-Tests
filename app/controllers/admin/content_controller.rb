...
class Admin::ContentController < Admin::BaseController
  ...
  def merge
    article1_id = params[:article_id]
    article2_id = params[:merge_with]
    
    if article2_id.blank? || (article2_id =~ /^[0-9]+$/.nil?)
      flash[:error] = 'Merge ID needs to be a positive integer'
      redirect_to :action => 'edit', :id => article1_id
      return
    end
    
    if article1_id == article2_id 
      flash[:error] = 'You need to enter a DIFFERENT ID in order to merge'
      redirect_to :action => 'edit', :id => article1_id
      return
    end
    
    article1 = Article.find_by_id(article1_id)
    new_article = article1.merge_with(article2_id)
    
    if new_article.nil?
      flash[:error] = "Article ID #{article2_id} does not exist"
      redirect_to :action => 'edit', :id => article1_id
      return
    end
    
    flash[:notice] = "Articles #{article1_id} and #{article2_id} were successfully merged"
